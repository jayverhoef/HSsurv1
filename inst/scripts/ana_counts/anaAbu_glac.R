...
load(paste0(basedir,'data/MHO_glac.rda'))
load(paste0(basedir,'data/dGlac.rda'))
...
MHO = MHO_glac
file_name = 'Anafigs_glac'
dstk = dGlac
# rename column so consistent with terrestrial datasets
names(dstk)[5] = 'count'
# remove records with missing count values
dstk = dstk[!is.na(dstk$count),]
# the maximum year for modeling, set the number of years
# beginning in 1996
nyr = 2023 - 1995
...
# get stockID's with polyid's.
pidstock = dstk[!duplicated(dstk[,'polyid']),c('polyid','stockid')]
polyid_gt0 = NULL
polyid_0 = NULL
# all polyids should have at least one count greater than 0
...
# design matrix
Xt <- model.matrix(count ~ dystd + I(dystd^2) +
...
tau = coef(glm(I(round(count,0)) ~ polyid - 1, 
...
Mana_glac = M
save(Mana_glac, file = paste0(basedir,'data/Mana_glac.rda'))
