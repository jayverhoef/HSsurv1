...
load(paste0(basedir,'data/dHOglac.rda'))
dHO = dHOglac
...
file_name = 'HOfigs_glac'

# ------------------------------------------------------------------------------
#                      Preliminaries
# ------------------------------------------------------------------------------

# just for graphics, read in survey data
dstk = dGlac
...
X <- model.matrix(y ~ dystd + I(dystd^2) + 
  hrstd + I(hrstd^2), data = dHO)
cbind(X[,4],dHO$hrstd)
# use Bernouli distribution to get some initial parameters assuming independence
...
optout1 = optim(c(.5,.5,-.5,.5,-.5), LLho, y = y, X = X)
...
# save the results to disk
 MHO_glac = M
 save(MHO_glac, file = paste0(basedir,'data/MHO_glac.rda'))
