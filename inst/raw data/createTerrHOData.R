...
#read in the data
load(paste0(basedir,'inst/raw data/tbl_percent_locs.rda'))
# copy data into working data.frame
dt = tbl_percent_locs
dt = as.data.frame(dt[,1:16])
...
# remove records with missing tide data
dt = dt[!is.na(dt$minutes_from_low),]
...
dt = dt[dt$minutes_from_low > -300 & 
  dt$minutes_from_low < 300,]
...
dt$tdstd = dt$minutes_from_low/150
dt$hrstd = (dt$solhr - 12)/6

# save dataset
dHOterr = dt
save(dHOterr, file = paste0(basedir,'/data/dHOterr.rda'))
